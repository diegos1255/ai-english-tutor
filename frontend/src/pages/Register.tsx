  ...
  const checkEmailExistence = async () => {
    ...
    try {
      ...
      if (response.data.exists) {
        setEmailWarning(t('auth.email_warning_text'));
      } else {
        setEmailWarning(null); 
      }
    } catch (error) {
  ...
  return (
    <div className="min-h-screen bg-gray-50 flex flex-col font-sans">
      ...
      <main className="flex-1 flex justify-center items-center px-4 py-8">
        <div className="w-full max-w-3xl bg-white rounded-2xl shadow-xl border border-gray-100 overflow-hidden transition-all duration-300">
          
          <div className="px-8 py-4 border-b border-gray-100 flex justify-between items-center bg-white">
            <h2 className="text-lg font-bold text-gray-900">{t('auth.register_title')}</h2>
          </div>
          ...
          <div className="p-8">
            ...
            <div className="mb-10 mt-2 px-4 w-full">
              <div className="flex items-center justify-between relative">
                ...
                <div 
                  className="absolute left-0 top-1/2 -translate-y-1/2 h-1.5 bg-blue-600 rounded-full -z-10 transition-all duration-500 ease-out" 
                ...
                <div className={`flex flex-col items-center bg-white px-2`}>
                  ...
                  <span className={`absolute -bottom-6 text-sm font-semibold ${step >= 1 ? 'text-blue-600' : 'text-gray-400'}`}>{t('auth.step_personal')}</span>
                </div>
                ...
                <div className={`flex flex-col items-center bg-white px-2`}>
                  ...
                  <span className={`absolute -bottom-6 text-sm font-semibold ${step >= 2 ? 'text-blue-600' : 'text-gray-400'}`}>{t('auth.step_account')}</span>
                </div>
                ...
                <div className={`flex flex-col items-center bg-white px-2`}>
                  ...
                  <span className={`absolute -bottom-6 text-sm font-semibold ${step === 3 ? 'text-blue-600' : 'text-gray-400'}`}>{t('auth.step_review')}</span>
                </div>
              </div>
            </div>
            ...
            <div className="space-y-5 min-h-[220px] mt-10">
              ...
              {step === 1 && (
                <div className="space-y-5 animate-fade-in">
                  <div>
                    <label className="block text-sm font-medium text-gray-900 mb-1.5">{t('auth.name_label')}</label>
                    <input type="text" value={name} onChange={(e) => setName(e.target.value)} required className="w-full px-4 py-2.5 bg-gray-50 border border-gray-200 rounded-lg focus:ring-2 focus:ring-blue-500 focus:bg-white outline-none transition" placeholder={t('auth.name_placeholder')} />
                  </div>
                  <div>
                    <label className="block text-sm font-medium text-gray-900 mb-1.5">{t('auth.birth_date_label')} <span className="text-gray-400 font-normal">{t('auth.optional')}</span></label>
                    <input type="date" value={birthDate} onChange={(e) => setBirthDate(e.target.value)} className="w-full px-4 py-2.5 bg-gray-50 border border-gray-200 rounded-lg focus:ring-2 focus:ring-blue-500 focus:bg-white outline-none transition text-gray-700" />
                  </div>
                </div>
              )}
              ...
              {step === 2 && (
                <div className="space-y-5 animate-fade-in">
                  ...
                  <div>
                    ...
                    {emailWarning && (
                      <div className="mt-2.5 flex items-start gap-2 p-3 bg-orange-50 border border-orange-200 rounded-lg text-orange-800 text-sm animate-fade-in">
                        ...
                        <div>
                          {/*<span className="font-semibold block mb-0.5">{t('auth.email_warning_title')}</span> */}
                          {emailWarning}
                        </div>
                      </div>
                    )}
                  ...
                  <div className="grid grid-cols-1 sm:grid-cols-2 gap-5">
                    <div>
                      <label className="block text-sm font-medium text-gray-900 mb-1.5">{t('auth.create_password_label')}</label>
                      <div className="relative">
                        <input type={showPassword ? "text" : "password"} value={password} onChange={(e) => setPassword(e.target.value)} required className="w-full px-4 pr-10 py-2.5 bg-gray-50 border border-gray-200 rounded-lg focus:ring-2 focus:ring-blue-500 focus:bg-white outline-none transition" placeholder={t('auth.password_placeholder')} />
                      ...
                      <p className="text-xs text-gray-500 mt-1.5">{t('auth.password_hint')}</p>
                    </div>

                    <div>
                      <label className="block text-sm font-medium text-gray-900 mb-1.5">{t('auth.confirm_password_label')}</label>
                      <div className="relative">
                        <input type={showPassword ? "text" : "password"} value={confirmPassword} onChange={(e) => setConfirmPassword(e.target.value)} required className={`w-full px-4 pr-10 py-2.5 bg-gray-50 border rounded-lg focus:ring-2 focus:bg-white outline-none transition ${passwordsMismatch ? 'border-red-500 focus:ring-red-500' : 'border-gray-200 focus:ring-blue-500'}`} placeholder={t('auth.password_placeholder')} />
                        <button type="button" onClick={() => setShowPassword(!showPassword)} className="absolute inset-y-0 right-0 pr-3 flex items-center text-gray-400 hover:text-gray-600 transition">
                          {showPassword ? (
                            <svg className="w-5 h-5" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M13.875 18.825A10.05 10.05 0 0112 19c-4.478 0-8.268-2.943-9.543-7a9.97 9.97 0 011.563-3.029m5.858.908a3 3 0 114.243 4.243M9.878 9.878l4.242 4.242M9.88 9.88l-3.29-3.29m7.532 7.532l3.29 3.29M3 3l3.59 3.59m0 0A9.953 9.953 0 0112 5c4.478 0 8.268 2.943 9.543 7a10.025 10.025 0 01-4.132 5.411m0 0l-3.41-3.41" /></svg>
                          ) : (
                      ...
                      {passwordsMismatch && (<p className="text-xs text-red-500 mt-1.5 font-medium">{t('auth.passwords_mismatch')}</p>)}
                    </div>
                  </div>
                </div>
              )}
              ...
              {step === 3 && (
                <div className="space-y-5 animate-fade-in">
                  <div className="bg-blue-50 text-blue-800 p-4 rounded-xl text-sm border border-blue-100 flex items-start gap-3">
                    ...
                    <div>
                      <p className="font-bold text-base mb-1">{t('auth.almost_there')}</p>
                      <p>{t('auth.review_text')}</p>
                    </div>
                  </div>
                  <div className="bg-gray-50 p-5 rounded-xl border border-gray-100 space-y-3 shadow-inner">
                    <div className="flex justify-between border-b border-gray-100 pb-2.5">
                      <p className="text-sm text-gray-500">{t('auth.name_label')}</p>
                      <p className="font-semibold text-gray-900">{name}</p>
                    </div>
                    <div className="flex justify-between border-b border-gray-100 pb-2.5">
                      <p className="text-sm text-gray-500">{t('auth.email_label')}</p>
                      <p className="font-semibold text-gray-900">{email}</p>
                    </div>
                    {birthDate && (
                      <div className="flex justify-between">
                        <p className="text-sm text-gray-500">{t('auth.birth_date_label')}</p>
            ...
            {step < 3 && (
              <div className="mt-8 text-sm text-gray-500 text-center border-t border-gray-100 pt-6">
                {t('auth.has_account')} <Link to="/login" className="text-blue-600 hover:underline font-semibold">{t('auth.login_link')}</Link>
              </div>
            )}
